...
public class TagParser {
	private static final String INSERT_SCHEMA = "(`course`, `course_name`, `section`, `class_nbr`, `units`, `type`, `days`, `times`, `location`, `instructor`)";
	private static final String CREATE_TABLE_SCHEMA = "(course varchar(255),course_name varchar(255),section int,class_nbr int,units varchar(255),type varchar(255),days varchar(255),times varchar(255),location varchar(255),instructor varchar(255))";
	...
	public static void toSQL(String filename) {
		...
		try { // DB name, needs to match with your db
			output.write("use SJSU\n");
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		while (in.hasNextLine()) {
			...
			if (current[0].contains("<ParaStyle:Division>")) { // reads the
				...
				try {
					output.write("create table " + currentTable
							+ CREATE_TABLE_SCHEMA + ";\nCOMMIT;\n");
				} catch (Exception e) {
					e.printStackTrace();
				}
